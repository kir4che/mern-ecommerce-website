...
const OrderItemDisplay = ({
  ...
  return (
    <li className="flex w-full gap-x-4" key={productId}>
      <img
...
const Checkout = () => {
  ...
  if (!isAuthenticated && !isLoadingCart && !isLoadingOrder) {
    return (
      <>
        <div className="flex-center min-h-screen">
          <div className="text-center space-y-4">
            <h2 className="text-2xl font-bold">需要登入</h2>
            <p className="text-gray-600">請先登入會員帳戶才能進入結帳頁面</p>
            <button
              onClick={() =>
                openConfirmDialog({
                  title: "需要登入",
  ...
  if (isRedirectingToPayment) return <Loading fullPage />;
