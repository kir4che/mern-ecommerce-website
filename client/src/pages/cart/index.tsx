...
const Cart = () => {
  ...
  const handleOrderPlaced = async () => {
    ...
    if (!BuyerInfo.name || !BuyerInfo.phone || !BuyerInfo.address) return alert('請填寫完整資訊！');
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/orders`, {
        ...
        body: JSON.stringify({
          ...
          totalAmount: cart.reduce((acc, item) => acc + item.product.price * item.quantity, 0) + 65,
        }),
      });
    ...
    <Layout>
      <div className="flex md:flex-row flex-col p-10 px-[5vw]">
        <section className='space-y-8 md:w-3/4'>
          <section className="px-10 py-6 bg-white shadow-md">
            <div className="flex items-center justify-between">
              ...
              <Link to='/collections/all' className="text-sm hover:underline">繼續購物</Link>
            </div>
          ...
          <section>
            <h3 className="pb-2 mb-4 text-base border-b border-zinc-300">購買人資訊</h3>
        ...
        <section className="px-[2vw] md:w-1/4 pt-8">
          <h1 className="pb-4 mb-4 text-xl font-medium border-b border-zinc-300">訂單摘要</h1>
          <div>
            <p className="flex justify-between py-2 text-sm font-medium">
              <span>小計</span>
              <span className='font-semibold'>NT${cart.length > 0 && cart.reduce((acc, item) => acc + item.product.price * item.quantity, 0)}</span>
            </p>
            <p className="flex justify-between py-2 text-sm font-medium">
              <span>運費</span>
          ...
          <p className="flex justify-between pt-4 pb-10 mt-8 text-sm font-medium border-t border-zinc-300">
            <span>總計</span>
            <span className='font-semibold'>NT${cart.length > 0 && cart.reduce((acc, item) => acc + item.product.price * item.quantity, 0) + 65}</span>
          </p>
          <button className="w-full py-3 text-sm font-medium border-2 rounded border-primary hover:bg-secondary hover:text-primary text-secondary bg-primary" onClick={handleOrderPlaced}>前往付款</button>
        </section>
      </div>
      <div>
      </div >
