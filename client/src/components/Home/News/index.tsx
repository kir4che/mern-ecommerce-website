  ...
  return (
    <section className='py-10 overflow-hidden'>
      <div className='flex px-[2.5vw] md:px-0 items-center justify-between pt-2 border-t border-primary/30 mx-[2.5vw]'>
        ...
        <Link to='/blogs/news' className='text-sm hover:underline'>View all</Link>
      </div>
          ...
          posts && posts.slice(0, 3).map((post, index) => (
            <li key={index}>
              <Link to={`/blogs/news/${post._id}`} className='flex items-center gap-y-4 md:gap-y-0 py-4 border-b-[1.5px] md:border-b-[0.875px] border-dashed border-primary'>
                ...
                <p className='inline-block w-full text-base duration-500 ease-in-out md:text-sm md:inline hover:opacity-60'>{post.title}</p>
              </Link>
            </li>
          ))
        }
