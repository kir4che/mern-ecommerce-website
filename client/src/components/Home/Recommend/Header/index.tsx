...
const RecommendHeader = () => {
  ...
  return (
    <div className='items-center justify-between overflow-hidden sm:border-b border-primary sm:flex'>
      <motion.div variants={marqueeVariants} className="flex items-center w-screen h-22 sm:flex sm:h-14 xl:h-18" animate="animate">
        <div className='absolute flex gap-5'>
          {
            [...Array(8)].map((_, i) => (
              <p className='space-x-3 min-w-fit' key={i}>
                <span className='text-3xl font-medium sm:font-normal sm:text-1.5xl'>Recommend</span>
      ...
      <select className='z-0 block mt-2 ml-auto font-medium bg-white cursor-pointer sm:mt-0 w-fit min-w-44 sm:text-sm' onChange={handleSelectChange}>
