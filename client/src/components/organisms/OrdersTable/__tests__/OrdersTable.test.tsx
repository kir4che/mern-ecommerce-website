...
const useCancelOrderMutationMock = vi.mocked(useCancelOrderMutation);
...
describe("OrdersTable 元件", () => {
  ...
  beforeEach(() => {
    ...
    useCancelOrderMutationMock.mockReturnValue([
      vi.fn(),
      { isLoading: false, reset: vi.fn() },
    ] as never);
  });
  ...
  test("未支付的使用者訂單重定向至結帳", () => {
    ...
    const repayButton = screen.getByRole("button", { name: "前往付款" });
