...
const QuantityInput = ({ productId, quantity, max = 99, changeQuantity }) => (
  <div className='inline-flex px-3 border-[0.96px] border-primary'>
    <button onClick={() => changeQuantity(productId, quantity - 1)} disabled={quantity <= 1}>
      <svg className="w-2 text-gray-500 fill-current" viewBox="0 0 448 512">
        <path d="M416 208H32c-17.67 0-32 14.33-32 32v32c0 17.67 14.33 32 32 32h384c17.67 0 32-14.33 32-32v-32c0-17.67-14.33-32-32-32z" />
      </svg>
    </button>
    <input className="w-[3.6vw] min-w-8 text-sm py-1 mx-2 text-center border-none" type="text" value={quantity} min={1} max={max} />
    <button onClick={() => changeQuantity(productId, quantity + 1)} disabled={quantity >= max}>
      <svg className="w-2 text-gray-500 fill-current" viewBox="0 0 448 512">
...
const CartItem = () => {
  const { cart, removeFormCart, changeQuantity } = useCart()
          ...
          <div className="flex w-full gap-[2.5vw] md:gap-[1.5vw]">
            ...
            <div className='space-y-2'>
              ...
              <div className='flex items-center gap-[2vw] md:hidden'>
                {QuantityInput({ productId: item.productId, quantity: item.quantity, max: item.product.countInStock, changeQuantity })}
                {RemoveBtn({ onClick: () => removeFormCart(item._id) })}
              </div>
            </div>
          </div>
